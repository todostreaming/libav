...
void ff_ ## OPNAME ## _dirac_pixels8_c(uint8_t *dst,                    \
                                       const uint8_t *src[5],           \
                                       int stride, int h)               \
{                                                                       \
    ff_ ## OPNAME ## _pixels8_8_c(dst, src[0], stride, h);              \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels16_c(uint8_t *dst,                   \
                                        const uint8_t *src[5],          \
                                        int stride, int h)              \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_8_c(dst, src[0], stride, h);             \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels32_c(uint8_t *dst,                   \
                                        const uint8_t *src[5],          \
                                        int stride, int h)              \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_8_c(dst, src[0], stride, h);             \
    ff_ ## OPNAME ## _pixels16_8_c(dst + 16, src[0] + 16, stride, h);   \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels8_l2_c(uint8_t *dst,                 \
                                          const uint8_t *src[5],        \
                                          int stride, int h)            \
{                                                                       \
    ff_ ## OPNAME ## _pixels8_l2_8(dst, src[0], src[1], stride, stride, \
                                   stride, h);                          \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels16_l2_c(uint8_t *dst,                \
                                           const uint8_t *src[5],       \
                                           int stride, int h)           \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_l2_8(dst, src[0], src[1], stride,        \
                                    stride, stride, h);                 \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels32_l2_c(uint8_t *dst,                \
                                           const uint8_t *src[5],       \
                                           int stride, int h)           \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_l2_8(dst, src[0], src[1], stride,        \
                                    stride, stride, h);                 \
    ff_ ## OPNAME ## _pixels16_l2_8(dst + 16, src[0] + 16, src[1] + 16, \
                                    stride, stride, stride, h);         \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels8_l4_c(uint8_t *dst,                 \
                                          const uint8_t *src[5],        \
                                          int stride, int h)            \
{                                                                       \
    ff_ ## OPNAME ## _pixels8_l4_8(dst, src[0], src[1], src[2], src[3], \
                                   stride, stride, stride, stride,      \
                                   stride, h);                          \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels16_l4_c(uint8_t *dst,                \
                                           const uint8_t *src[5],       \
                                           int stride, int h)           \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_l4_8(dst, src[0], src[1], src[2],        \
                                    src[3], stride, stride, stride,     \
                                    stride, stride, h);                 \
}                                                                       \
void ff_ ## OPNAME ## _dirac_pixels32_l4_c(uint8_t *dst,                \
                                           const uint8_t *src[5],       \
                                           int stride, int h)           \
{                                                                       \
    ff_ ## OPNAME ## _pixels16_l4_8(dst, src[0], src[1], src[2],        \
                                    src[3], stride, stride, stride,     \
                                    stride, stride, h);                 \
    ff_ ## OPNAME ## _pixels16_l4_8(dst + 16, src[0] + 16, src[1] + 16, \
...
{                                                                       \
    ...
    while (h--) {                                                       \
        for (x = 0; x < WIDTH; x++) {                                   \
            OP(dst[x], (s0[x] * w[0] + s1[x] * w[1] + s2[x] *           \
                        w[2] + s3[x] * w[3] + 8) >> 4);                 \
        }                                                               \
                                                                        \
        dst += stride;                                                  \
