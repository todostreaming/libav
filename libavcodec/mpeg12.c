        ...
        0)
    {
        ...
        //MPEG-2 fps
            ...
            if (s->aspect_ratio_info > 1) {
                AVRational dar;{ AVRational tmp__0 = {s1->pan_scan.width, s1->pan_scan.height};  { AVRational tmp__1 = {s->width, s->height}; dar =
                ...
                // issue1613, 621, 562
                { { AVRational tmp__3 = {16, 9}; AVRational tmp__2 = {4, 3}; if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
                   (av_cmp_q(dar, tmp__2) && av_cmp_q(dar, tmp__3))) {
                    { AVRational tmp__4 = {s->width, s->height}; s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                                 tmp__4); }
                } else {
                    { AVRational tmp__5 = {s1->pan_scan.width, s1->pan_scan.height}; s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                                 tmp__5); } }}
...
AVCodec ff_mpeg1video_decoder = {
    ...
    0, 0, ONLY_IF_THREADS_ENABLED(mpeg_decode_update_thread_context)
,
...
AVCodec ff_mpeg2video_decoder = {
    ...
    CODEC_CAP_DRAW_HORIZ_BAND | CODEC_CAP_DR1 |
                      CODEC_CAP_TRUNCATED | CODEC_CAP_DELAY |
                      CODEC_CAP_SLICE_THREADS,
    0, 0, 0, 0, 0, 0, 0, NULL_IF_CONFIG_SMALL(mpeg2_video_profiles),
    sizeof(Mpeg1Context),
