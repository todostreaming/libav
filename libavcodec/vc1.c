 ...
 * @return Status
 */
int ff_vc1_decode_sequence_header(AVCodecContext *avctx, VC1Context *v,
                                  BitstreamContext *bc)
{
    ...
    v->s.max_b_frames   =
    avctx->max_b_frames = bitstream_read(bc, 3);  // common
    v->quantizer_mode   = bitstream_read(bc, 2);  // common
    v->finterpflag      = bitstream_read_bit(bc); // common

    if (v->res_sprite) {
        v->s.avctx->width        =
        v->s.avctx->coded_width  = bitstream_read(bc, 11);
        v->s.avctx->height       =
        v->s.avctx->coded_height = bitstream_read(bc, 11);
...
int ff_vc1_decode_entry_point(AVCodecContext *avctx, VC1Context *v,
                              BitstreamContext *bc)
{
    int i;
    ...
    if (bitstream_read_bit(bc)) {
        avctx->width        =
        avctx->coded_width  = (bitstream_read(bc, 12) + 1) << 1;
        avctx->height       =
        avctx->coded_height = (bitstream_read(bc, 12) + 1) << 1;
    }
    if (v->extended_mv)
