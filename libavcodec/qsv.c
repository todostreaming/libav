...
static int alloc_frame(AVCodecContext *avctx, QSVFrame *frame)
{
    ...
    if (frame->frame->format == AV_PIX_FMT_QSV) {
        memcpy(&frame->surface, frame->frame->data[3], sizeof(frame->surface));
    } else {
        frame->surface.Info.BitDepthLuma   = 8;
        frame->surface.Info.BitDepthChroma = 8;
        frame->surface.Info.FourCC         = MFX_FOURCC_NV12;
        frame->surface.Info.Width          = avctx->coded_width;
        frame->surface.Info.Height         = avctx->coded_height;
        frame->surface.Info.ChromaFormat   = MFX_CHROMAFORMAT_YUV420;
