...
int ff_qsv_enc_init(AVCodecContext *avctx, QSVEncContext *q)
{
    ...
    if (ret < 0) {
        av_log(avctx, AV_LOG_INFO, "MFXVideoENCODE_QueryIOSurf():%d\n", ret);
        return ff_qsv_error(ret);
    }

    if (ret = MFXVideoENCODE_Init(q->session, &q->param)) {
        av_log(avctx, AV_LOG_INFO, "MFXVideoENCODE_Init():%d\n", ret);
        return ff_qsv_error(ret);
    }
    ...
    /*
    av_log(q, AV_LOG_INFO, "sps");
    for (int i = 0; i < 64; i+=4)
        av_log(q, AV_LOG_INFO, ":%02x%02x%02x%02x", q->spspps[0][i], q->spspps[0][i+1], q->spspps[0][i+2], q->spspps[0][i+3]);
    av_log(q, AV_LOG_INFO, "\n");

    av_log(q, AV_LOG_INFO, "pps");
    for (int j = 0; j < 64; j+=4)
        av_log(q, AV_LOG_INFO, ":%02x%02x%02x%02x", q->spspps[1][j], q->spspps[1][j+1], q->spspps[1][j+2], q->spspps[1][j+3]);
    av_log(q, AV_LOG_INFO, "\n");
    */
                     ...
                     AVPacket *pkt, const AVFrame *frame, int *got_packet)
{
    ...
    *got_packet = 0;
    ...
    if (frame) {
        ...
        av_log(q, AV_LOG_INFO, "frame:NULL\n");
