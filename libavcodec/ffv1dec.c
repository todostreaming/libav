        ...
        for (i = 0; i < f->plane_count; i++) {
            ...
            if (fssrc->ac) {
                ...
                if (!pdst->vlc_state) {
                    int j;
                    for (j = 0; j < i; j++) {
                        av_freep(&fsdst->plane[i].vlc_state);
                    }
                    return AVERROR(ENOMEM);
                }
...
fail:
    for (i = 0; i < f->quant_table_count; i++)
        av_freep(f->initial_states + i);

    av_frame_free(&f->picture.f);
    av_frame_free(&f->last_picture.f);
    return ret;
}
