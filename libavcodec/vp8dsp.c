...
#define INIT_DSP(vpn)                                                   \
av_cold void ff_ ## vpn ## dsp_init(VP8DSPContext * dsp)                \
{                                                                       \
    dsp->vp8_luma_dc_wht    = vpn ## _luma_dc_wht_c;                    \
    dsp->vp8_luma_dc_wht_dc = vpn ## _luma_dc_wht_dc_c;                 \
    dsp->vp8_idct_add       = vpn ## _idct_add_c;                       \
    dsp->vp8_idct_dc_add    = vpn ## _idct_dc_add_c;                    \
    dsp->vp8_idct_dc_add4y  = vpn ## _idct_dc_add4y_c;                  \
    dsp->vp8_idct_dc_add4uv = vpn ## _idct_dc_add4uv_c;                 \
                                                                        \
    dsp->vp8_v_loop_filter16y = vpn ## _v_loop_filter16_c;              \
    dsp->vp8_h_loop_filter16y = vpn ## _h_loop_filter16_c;              \
    dsp->vp8_v_loop_filter8uv = vpn ## _v_loop_filter8uv_c;             \
    dsp->vp8_h_loop_filter8uv = vpn ## _h_loop_filter8uv_c;             \
                                                                        \
    dsp->vp8_v_loop_filter16y_inner = vpn ## _v_loop_filter16_inner_c;  \
    dsp->vp8_h_loop_filter16y_inner = vpn ## _h_loop_filter16_inner_c;  \
    dsp->vp8_v_loop_filter8uv_inner = vpn ## _v_loop_filter8uv_inner_c; \
    dsp->vp8_h_loop_filter8uv_inner = vpn ## _h_loop_filter8uv_inner_c; \
                                                                        \
    dsp->vp8_v_loop_filter_simple = vpn ## _v_loop_filter_simple_c;     \
    dsp->vp8_h_loop_filter_simple = vpn ## _h_loop_filter_simple_c;     \
                                                                        \
    VP8_MC_FUNC(0, 16);                                                 \
    VP8_MC_FUNC(1, 8);                                                  \
    VP8_MC_FUNC(2, 4);                                                  \
    ...
    VP8_BILINEAR_MC_FUNC(2, 4);                                         \
                                                                        \
    if (ARCH_ARM)                                                       \
        ff_ ## vpn ## dsp_init_arm(dsp);                                \
    if (ARCH_PPC)                                                       \
        ff_ ## vpn ## dsp_init_ppc(dsp);                                \
    if (ARCH_X86)                                                       \
        ff_ ## vpn ## dsp_init_x86(dsp);                                \
}
