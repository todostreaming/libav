...
#define GET_MQUANT()                                           \
    if (v->dquantfrm) {                                        \
        ...
        if (v->dqprofile == DQPROFILE_ALL_MBS) {               \
            if (v->dqbilevel) {                                \
                mquant = bitstream_read_bit(bc) ? v->altpq     \
                                                : v->pq;       \
            } else {                                           \
                              ...
                              int coded, int codingset)
{
    ...
    if (dcdiff) {
        if (dcdiff == 119 /* ESC index value */) {
            /* TODO: Optimize */
            if (v->pq == 1)
                dcdiff = bitstream_read(bc, 10);
            else if (v->pq == 2)
                dcdiff = bitstream_read(bc, 9);
            else
                dcdiff = bitstream_read(bc, 8);
        } else {
            if (v->pq == 1)
...
static int vc1_decode_p_mb_intfr(VC1Context *v)
{
    ...
    if (!skipped) {
        ...
        if (ff_vc1_mbmode_intfrp[v->fourmvswitch][idx_mbmode][0] == MV_PMODE_INTFR_INTRA) { // intra MB
            ...
            fieldtx                  =
            v->fieldtx_plane[mb_pos] = bitstream_read_bit(bc);
...
static int vc1_decode_p_mb_intfi(VC1Context *v)
{
    ...
    if (idx_mbmode <= 1) { // intra MB
        ...
        s->c_dc_scale = s->c_dc_scale_table[mquant];
        v->s.ac_pred  =
        v->acpred_plane[mb_pos] = bitstream_read_bit(bc);
...
static void vc1_decode_b_mb_intfi(VC1Context *v)
{
    ...
    if (idx_mbmode <= 1) { // intra MB
        ...
        if (v->fmb_is_raw)
            fwd                         =
            v->forward_mb_plane[mb_pos] = bitstream_read_bit(bc);
        else
