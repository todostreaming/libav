                             ...
                             HEVCDecoderConfigurationRecord *hvcc)
{
    ...
    ret = bitstream_init8(&bc, rbsp_buf, rbsp_size);
    if (ret < 0)
        goto end;

    nal_unit_parse_header(&bc, &nal_type);
    ...
    case NAL_SEI_SUFFIX:
        ...
        if (ret < 0)
            ...
            ret = hvcc_parse_vps(&bc, hvcc);
        else if (nal_type == NAL_SPS)
            ret = hvcc_parse_sps(&bc, hvcc);
        else if (nal_type == NAL_PPS)
            ret = hvcc_parse_pps(&bc, hvcc);
        if (ret < 0)
            goto end;
        break;
    default:
