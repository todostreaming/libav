...
{"content_type", "set a specific content type", OFFSET(content_type), AV_OPT_TYPE_STRING, { 0 }, 0, 0, D|E },
...
{"mime_type", "set MIME type", OFFSET(mime_type), AV_OPT_TYPE_STRING, {0}, 0, 0, AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY  },
...
static int check_http_code(URLContext *h, int http_code, char *end)
{
    HTTPContext *s = h->priv_data;
...
static int parse_location(HTTPContext *s, char *p)
{
...
static int parse_icy(HTTPContext *s, char *p, char *tag)
{
